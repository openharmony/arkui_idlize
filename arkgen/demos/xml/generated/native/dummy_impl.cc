...
namespace OHOS::Ace::NG::GeneratedModifier {
    ...
    namespace XmlSerializerAccessor {
    void DestroyPeerImpl(XmlSerializerPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("destroyPeer(");
    ...
    OH_NativePointer CtorImpl(const OH_Buffer* buffer,
                              const Opt_String* encoding)
    {
        if (!needGroupedLog(1))
            return (XmlSerializerPeer*) 100;
        string out("new XmlSerializer(");
        WriteToString(&out, buffer);
        out.append(", ");
        WriteToString(&out, encoding);
        out.append(") \n");
        out.append("[return (XmlSerializerPeer*) 100] \n");
        appendGroupedLog(1, out);
        return (XmlSerializerPeer*) 100;
    }
    ...
    void SetAttributesImpl(XmlSerializerPeer* peer,
                           const OH_String* name,
                           const OH_String* value)
    {
        if (!needGroupedLog(1))
            return;
        string out("setAttributes(");
    ...
    void AddEmptyElementImpl(XmlSerializerPeer* peer,
                             const OH_String* name)
    {
        if (!needGroupedLog(1))
            return;
        string out("addEmptyElement(");
        WriteToString(&out, name);
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    void SetDeclarationImpl(XmlSerializerPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("setDeclaration(");
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    void StartElementImpl(XmlSerializerPeer* peer,
                          const OH_String* name)
    {
        if (!needGroupedLog(1))
            return;
        string out("startElement(");
        WriteToString(&out, name);
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    void EndElementImpl(XmlSerializerPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("endElement(");
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    void SetNamespaceImpl(XmlSerializerPeer* peer,
                          const OH_String* prefix,
                          const OH_String* namespace)
    {
        if (!needGroupedLog(1))
            return;
        string out("setNamespace(");
        WriteToString(&out, prefix);
        out.append(", ");
        WriteToString(&out, namespace);
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    void SetCommentImpl(XmlSerializerPeer* peer,
                        const OH_String* text)
    {
        if (!needGroupedLog(1))
            return;
        string out("setComment(");
        WriteToString(&out, text);
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    void SetCDATAImpl(XmlSerializerPeer* peer,
                      const OH_String* text)
    {
        if (!needGroupedLog(1))
            return;
    ...
    void SetTextImpl(XmlSerializerPeer* peer,
                     const OH_String* text)
    {
        if (!needGroupedLog(1))
            return;
    ...
    void SetDocTypeImpl(XmlSerializerPeer* peer,
                        const OH_String* text)
    {
        if (!needGroupedLog(1))
            return;
        string out("setDocType(");
        WriteToString(&out, text);
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    } // XmlSerializerAccessor
    namespace ParseInfoAccessor {
    void DestroyPeerImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("destroyPeer(");
    ...
    {
        if (!needGroupedLog(1))
            return (ParseInfoPeer*) 100;
        string out("new ParseInfo(");
        out.append(") \n");
        out.append("[return (ParseInfoPeer*) 100] \n");
        appendGroupedLog(1, out);
        return (ParseInfoPeer*) 100;
    }
    OH_NativePointer GetFinalizerImpl()
    ...
    OH_Int32 GetColumnNumberImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return 0;
        string out("getColumnNumber(");
        out.append(") \n");
        out.append("[return 0] \n");
        appendGroupedLog(1, out);
        return 0;
    }
    OH_Int32 GetDepthImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return 0;
        string out("getDepth(");
    ...
    OH_Int32 GetLineNumberImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return 0;
        string out("getLineNumber(");
        out.append(") \n");
        out.append("[return 0] \n");
        appendGroupedLog(1, out);
        return 0;
    }
    void GetNameImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("getName(");
    ...
    void GetNamespaceImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("getNamespace(");
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    void GetPrefixImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("getPrefix(");
        out.append(") \n");
        appendGroupedLog(1, out);
    }
    void GetTextImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("getText(");
    ...
    OH_Boolean IsEmptyElementTagImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return 0;
        string out("isEmptyElementTag(");
        out.append(") \n");
        out.append("[return 0] \n");
        appendGroupedLog(1, out);
        return 0;
    }
    OH_Boolean IsWhitespaceImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return 0;
        string out("isWhitespace(");
        out.append(") \n");
        out.append("[return 0] \n");
        appendGroupedLog(1, out);
        return 0;
    }
    OH_Int32 GetAttributeCountImpl(ParseInfoPeer* peer)
    {
        if (!needGroupedLog(1))
            return 0;
        string out("getAttributeCount(");
        out.append(") \n");
        out.append("[return 0] \n");
        appendGroupedLog(1, out);
        return 0;
    ...
    namespace XmlPullParserAccessor {
    void DestroyPeerImpl(XmlPullParserPeer* peer)
    {
        if (!needGroupedLog(1))
            return;
        string out("destroyPeer(");
    ...
    {
        if (!needGroupedLog(1))
            return (XmlPullParserPeer*) 100;
        string out("new XmlPullParser(");
        WriteToString(&out, buffer);
        out.append(", ");
        WriteToString(&out, encoding);
        out.append(") \n");
        out.append("[return (XmlPullParserPeer*) 100] \n");
        appendGroupedLog(1, out);
        return (XmlPullParserPeer*) 100;
    }
    OH_NativePointer GetFinalizerImpl()
    ...
    void ParseImpl(XmlPullParserPeer* peer,
                   const OH_XML_ParseOptions* option)
    {
        if (!needGroupedLog(1))
            return;
        string out("parse(");
    ...
    void ParseXmlImpl(XmlPullParserPeer* peer,
                      const OH_XML_ParseOptions* option)
    {
        if (!needGroupedLog(1))
            return;
    ...
    } // XmlPullParserAccessor
    const GENERATED_ArkUIXmlSerializerAccessor* GetXmlSerializerAccessor()
    {
        static const GENERATED_ArkUIXmlSerializerAccessor XmlSerializerAccessorImpl {
            XmlSerializerAccessor::DestroyPeerImpl,
            XmlSerializerAccessor::CtorImpl,
            XmlSerializerAccessor::GetFinalizerImpl,
            XmlSerializerAccessor::SetAttributesImpl,
            XmlSerializerAccessor::AddEmptyElementImpl,
            XmlSerializerAccessor::SetDeclarationImpl,
            XmlSerializerAccessor::StartElementImpl,
            XmlSerializerAccessor::EndElementImpl,
            XmlSerializerAccessor::SetNamespaceImpl,
            XmlSerializerAccessor::SetCommentImpl,
            XmlSerializerAccessor::SetCDATAImpl,
            XmlSerializerAccessor::SetTextImpl,
            XmlSerializerAccessor::SetDocTypeImpl,
        };
        return &XmlSerializerAccessorImpl;
    }

    struct XmlSerializerPeer {
        virtual ~XmlSerializerPeer() = default;
    };
    const GENERATED_ArkUIParseInfoAccessor* GetParseInfoAccessor()
    {
        static const GENERATED_ArkUIParseInfoAccessor ParseInfoAccessorImpl {
            ParseInfoAccessor::DestroyPeerImpl,
            ParseInfoAccessor::CtorImpl,
            ParseInfoAccessor::GetFinalizerImpl,
            ParseInfoAccessor::GetColumnNumberImpl,
            ParseInfoAccessor::GetDepthImpl,
            ParseInfoAccessor::GetLineNumberImpl,
            ParseInfoAccessor::GetNameImpl,
            ParseInfoAccessor::GetNamespaceImpl,
            ParseInfoAccessor::GetPrefixImpl,
            ParseInfoAccessor::GetTextImpl,
            ParseInfoAccessor::IsEmptyElementTagImpl,
            ParseInfoAccessor::IsWhitespaceImpl,
            ParseInfoAccessor::GetAttributeCountImpl,
        };
        return &ParseInfoAccessorImpl;
    }

    struct ParseInfoPeer {
        virtual ~ParseInfoPeer() = default;
    };
    const GENERATED_ArkUIXmlPullParserAccessor* GetXmlPullParserAccessor()
    {
        static const GENERATED_ArkUIXmlPullParserAccessor XmlPullParserAccessorImpl {
            XmlPullParserAccessor::DestroyPeerImpl,
            XmlPullParserAccessor::CtorImpl,
            XmlPullParserAccessor::GetFinalizerImpl,
            XmlPullParserAccessor::ParseImpl,
            XmlPullParserAccessor::ParseXmlImpl,
        };
        return &XmlPullParserAccessorImpl;
    }

    struct XmlPullParserPeer {
        virtual ~XmlPullParserPeer() = default;
    };
    const GENERATED_ArkUIAccessors* GENERATED_GetArkUIAccessors()
    {
        static const GENERATED_ArkUIAccessors accessorsImpl = {
            GetXmlSerializerAccessor,
            GetParseInfoAccessor,
            GetXmlPullParserAccessor,
        };
