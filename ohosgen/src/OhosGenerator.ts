...
interface DependecyCollector {
    ...
    parseImport(imp: IDLImport): void
    collect(decl: IDLNode, fileName?: string, traverse?: boolean): void
...
class ManyFilesDependecyCollector implements DependecyCollector{
    ...
    // file -> imports
    fileToImpors: Map<string, Set<IDLImport>> = new Map()
    ...
    collect(decl: IDLNode, fileName?: string, traverse: boolean = true) {
        if (!fileName) {
    ...
    private collectTypeReference(type?: IDLReferenceType): string {
        if (!type) {
        ...
        return isNamedNode(resolvedType) ? type.name : this.NONE_TYPE
    }

    NONE_TYPE: string = "NONE_TYPE"
    private collectInterface(decl: IDLInterface): string[] {
    ...
    private collectType(type: IDLType): string {
        if (isNamedNode(type)) {
        ...
        return isNamedNode(type) ? type.name : this.NONE_TYPE
    }
...
function writeCJMethod(writer: LanguageWriter, method: {name: string, method: NamedMethodSignature}) {
    ...
    writer.writeMethodImplementation(new Method((method.name.startsWith('_') ? '' : '_').concat(method.name), method.method, [MethodModifier.STATIC]), () => {
        ...
        for(let param of parameters.args) {
            if (isContainerType(param) || arrayLikeTypes.has(forceAsNamedNode(param).name)) {
            ...
            } else if (stringLikeTypes.has(forceAsNamedNode(param).name)) {
        ...
        } else if (returnType === IDLStringType) {
...
abstract class OHOSVisitor {
        ...
        ;((writer: LanguageWriter) => {
            this.interfaces.forEach(it => {
                ...
                this.getPropertiesFromInterfaces(it).concat(it.properties).forEach(property => {
                    ...
                    const setterSignature = makePeerCallSignature(this.library, [createParameter("value", property.type)], IDLVoidType, "self")
    ...
    private printStructsDeclarations(data: IDLInterface[]) {
        data.forEach(clazz => {
            const namespaces = getNamespacesPathFor(clazz);
    ...
    private printInterfacesDeclarations(data: IDLInterface[]) {
        data.forEach(int => {
            if (hasExtAttribute(int, IDLExtendedAttributes.GlobalScope)) {
                return
            }
            const superTypes = int.inheritance.filter(it => it !== IDLTopType).map(superClass => `${superClass.name}Interface`)
            const peerWriter = this.getPeerWriter(int)
            peerWriter.writeInterface(`${int.name}Interface`, writer => {
                int.properties.forEach(prop => {
                    writer.writeFieldDeclaration(prop.name, prop.type, [], isOptionalType(prop.type))
                })
                int.methods.forEach(method => {
                    if (method.isStatic) {
                        return
    ...
    private printInterfacesImplementations(data: Array<IDLInterface>) {
        data.forEach(int => {
            const namespaces = getNamespacesPathFor(int);
            const peerWriter = this.getPeerWriter(int)
            if (peerWriter.language != Language.CJ) namespaces.forEach(ns => peerWriter.pushNamespace(ns.name, true));
            ...
            peerWriter.writeClass(`${int.name}`, writer => {
                ...
                fields.forEach(f => {
                    const typeName = isNamedNode(f.type) ? f.type.name : "UnknownType"
                    ...
                    writer.writeMethodImplementation(new Method(`set${capitalize(f.name)}`,
                        new NamedMethodSignature(IDLVoidType, [f.type], [f.name])), writer => {
    ...
    abstract getPeerWriter(decl: IDLNode): LanguageWriter
...
class OneFileOHOSVisitor extends OHOSVisitor {
    ...
    getPeerWriter(decl: IDLNode): LanguageWriter {
...
function getFileNameFromDeclaration(decl: IDLNode): string {
