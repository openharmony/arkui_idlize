...
namespace OHOS::Ace::NG::GeneratedModifier {
    ...
    {
        auto peerImpl = reinterpret_cast<XmlSerializerPeerImpl *>(peer);
        if (peerImpl) {
            delete peerImpl;
        }
    }
    ...
    {
        return new XmlSerializerPeer(buffer, encoding);
    }
    ...
    {
        auto peerImpl = reinterpret_cast<ParseInfoPeerImpl *>(peer);
        if (peerImpl) {
            delete peerImpl;
        }
    }
    ...
    {
        auto peerImpl = reinterpret_cast<XmlPullParserPeerImpl *>(peer);
        if (peerImpl) {
            delete peerImpl;
        }
    }
    OH_NativePointer CtorImpl(const OH_Buffer* buffer,
                              const Opt_String* encoding)
    {
        return new XmlPullParserPeer(buffer, encoding);
    }
    OH_NativePointer GetFinalizerImpl()
    {
        return reinterpret_cast<void *>(&DestroyPeerImpl);
