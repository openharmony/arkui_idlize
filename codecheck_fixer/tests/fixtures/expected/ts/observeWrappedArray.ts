...
export class WrappedArray<T> extends Array<T> implements IObservedObject, ObserveWrappedBase, ISubscribedWatches {
     ...
     *
      * @param predicate A function that accepts up to three arguments. The filter method calls the predicate function
    ...
    /**
      * Makes a shallow copy of the Array part to another location in the same Array and returns it without modifying
      * its length.
     *
     ...
     *
      * @param callbackfn A function that accepts up to four arguments. The reduce method calls the callbackfn function
      * one time for each element in the array.
     ...
     *
      ...
      * first call to the callbackfn function provides this value as an argument instead of an array value.
     * @returns a result after applying callbackfn over all elements of the Array
     */
