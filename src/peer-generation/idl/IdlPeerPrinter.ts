...
import { ARK_MATERIALIZEDBASE_EMPTY_PARAMETER, ARKOALA_PACKAGE, ARKOALA_PACKAGE_PATH } from "../printers/lang/Java";
...
class PeerFileVisitor {
    ...
    protected printImports(printer: LanguageWriter, targetBasename: string): void {
        this.getDefaultPeerImports(this.printerContext.language)!.forEach(it => printer.print(it))
        ...
        if (this.printerContext.language === Language.TS
            || this.printerContext.language === Language.ARKTS) {
        ...
        if (this.printerContext.language !== Language.ARKTS) {
    ...
    protected printPeer(peer: IdlPeerClass, printer: LanguageWriter) {
        printer.writeClass(componentToPeerClass(peer.componentName), (writer) => {
            ...
            peer.methods
                .filter(m => writer.language !== Language.ARKTS || !PeerGeneratorConfig.ArkTsIgnoredMethods.includes(m.overloadedName))
                .forEach(m => this.printPeerMethod(m, writer))
            this.printApplyMethod(peer, writer)
        }, this.generatePeerParentName(peer))
    }

    printFile(): void {
        const printer = createLanguageWriter(this.printerContext.language)
        const targetBasename = renameDtsToPeer(path.basename(this.file.originalFilename), this.printerContext.language, false)
...
class JavaPeerFileVisitor extends PeerFileVisitor {
    constructor(library: IdlPeerLibrary, file: IdlPeerFile, printerContext: PrinterContext, dumpSerialized: boolean) {
...
export function printPeers(idlLibrary: IdlPeerLibrary, printerContext: PrinterContext, dumpSerialized: boolean): Map<TargetFile, string> {
    const visitor = new PeersVisitor(idlLibrary, printerContext, dumpSerialized)
    visitor.printPeers()
...
export function writePeerMethod(printer: LanguageWriter, method: IdlPeerMethod, printerContext: PrinterContext, dumpSerialized: boolean,
    ...
    switch (printerContext.language) {
        case Language.ARKTS:
        case Language.TS:
        ...
        if (returnType != Type.Void) {
            ...
            if (method.hasReceiver() && returnType === Type.This) {
                ...
                if (!method.hasReceiver()) {
                    ...
                    const obj = `new ${retType.name}(${signature.argsNames.map(_ => "undefined").join(", ")})`
