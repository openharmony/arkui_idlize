...
export class TSDeclConvertor implements DeclarationConvertor<void> {
    ...
    convertTypedef(node: idl.IDLTypedef): void {
        ...
        this.writer.print(`export declare type ${node.name}${maybeTypeArguments} = ${type};`)
    }
    ...
    convertInterface(node: idl.IDLInterface): void {
        ...
        let extendsClause = "///extends ?" ///this.extendsClause(node)
    ...
    // private declarationMembers(node: idl.IDLInterface): readonly (ts.MethodDeclaration)[] {
