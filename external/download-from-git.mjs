...
export function downloadFromGit(repoUrl, ref, repoDir) {
  const gitDir = `${repoDir}/.git`
  const git = `git --git-dir=${gitDir} --work-tree=${repoDir}`

  let repoRefKind
  if (ref.match(/^[0-9a-zA-Z]{40}$/)) repoRefKind = 'hash'
  else if (ref.startsWith("origin/")) repoRefKind = 'remote'
  ...
  let needFetch = true
  if (repoRefKind === 'hash') {
    const exists = exec(`${git} cat-file -e ${ref}`, false).code == 0
    needFetch = !exists
  }
  ...
  if (!currentRefs.includes(ref)) {
    ...
    console.log(`try to checkout ref: ${ref}`)
    exec(`${git} checkout ${ref}`)
  }

  // follow remote
  if (repoRefKind === 'local') {
    console.log(`try to forward local branch: ${ref}`)
