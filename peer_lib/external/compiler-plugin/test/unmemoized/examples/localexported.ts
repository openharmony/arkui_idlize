...
function entry(__memo_context: __memo_context_type, __memo_id: __memo_id_type) {
    const __memo_scope = __memo_context.scope<void>(__memo_id + "1___key_id_DIRNAME/localexported.ts");
    if (__memo_scope.unchanged)
        return __memo_scope.cached;
    localexported(__memo_context, __memo_id + "0___key_id_DIRNAME/localexported.ts", 17);
