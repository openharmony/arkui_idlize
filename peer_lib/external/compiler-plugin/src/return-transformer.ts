...
import { FunctionKind, isFunctionOrMethod, isMemoKind, isVoidOrNotSpecified, runtimeIdentifier, RuntimeNames } from './util'
...
export class ReturnTransformer extends ScopedVisitor<ts.TypeNode | undefined> {
    ...
    visitor(node: ts.Node): ts.Node {
        ...
        if (ts.isReturnStatement(node) && isMemoKind(this.currentKind())) {
            ...
            return !isVoidOrNotSpecified(functionReturnType)
                ? ts.factory.updateReturnStatement(node, createScopeRecacheExpression(expression))
