...
*/
import { pointer, nullptr, wrapCallback, callCallback, InteropNativeModule, registerNativeModuleLibraryName, loadInteropNativeModule, NativeBuffer, SerializerBase, DeserializerBase } from "@koalaui/interop"
...
import { ArkButtonPeer,
         ArkCalendarPickerPeer,
         ArkFormComponentPeer,
         ArkSideBarContainerPeer,
         ArkSideBarContainerComponent,
         ArkTabContentPeer,
         SubTabBarStyle,
         BottomTabBarStyle,
         ArkCommonMethodPeer,
} from "@arkoala/arkui/framework"
// Improve: It needs to be possible to use CanvasRenderingContext2D without import
import { CanvasRenderingContext2D, CanvasRenderingContext2D_serializer, CanvasRenderingContext2DInternal } from "@arkoala/arkui/framework"
...
import { ArkUIGeneratedNativeModule } from "./framework/peers/ArkUIGeneratedNativeModule"
import { mkdirSync, writeFileSync } from "fs"
import { CallbackKind } from "@arkoala/arkui/framework/peers/CallbackKind"
...
// Improve: Fix the tests according to the latest callback changes
...
function enqueueCallback(
    ...
    const deserializer = new DeserializerBase(serializer.asBuffer(), bufferLength)
...
// 
//     enqueueCallback(
//         createDefaultWriteCallback(CallbackKind.Kind_Callback_Boolean, (): boolean => {
//             return true
//         }),
//         (deserializer) => {
//             const callback = deserializer.readCallback_Boolean(true)
...
function checkWriteFunction() {
    ...
    // Improve: id is small number
    const id = s.getByte(0) | (s.getByte(1) << 8) | (s.getByte(2) << 16) | (s.getByte(3) << 24)
    s.release()
    const args = SerializerBase.hold()
    args.writeNumber(20)
    args.writeBoolean(true)
    // Improve: callCallback() result should be 30
...
function checkButton() {
    ...
    checkResult("width", () => peer.setWidthAttribute("42%"),
        `setWidth({.tag=INTEROP_TAG_OBJECT, .value={.selector=0, .value0={.selector=0, .value0={.chars="42%", .length=3}}}})`)

    checkResult("height", () => peer.setHeightAttribute(132),
        `setHeight({.tag=INTEROP_TAG_OBJECT, .value={.selector=0, .value0={.selector=1, .value1={.tag=102, .i32=132}}}})`)
    ...
    checkResult("labelStyle", () => peer.setLabelStyleAttribute({ maxLines: 3 }),
    ...
    checkResult("labelStyle2", () => peer.setLabelStyleAttribute({}),
...
function checkFormComponent() {
    ...
    let peer = ArkCommonMethodPeer.create()
    checkResult("size int", () => peer.setSizeAttribute({ width: 5, height: 6 }),
        `setSize({.tag=INTEROP_TAG_OBJECT, .value={.width={.tag=INTEROP_TAG_OBJECT, .value={.selector=1, .value1={.tag=102, .i32=5}}}, .height={.tag=INTEROP_TAG_OBJECT, .value={.selector=1, .value1={.tag=102, .i32=6}}}}})`)
    checkResult("size float", () => peer.setSizeAttribute({ width: 5.5, height: 6.789 }),
        `setSize({.tag=INTEROP_TAG_OBJECT, .value={.width={.tag=INTEROP_TAG_OBJECT, .value={.selector=1, .value1={.tag=103, .f32=5.5}}}, .height={.tag=INTEROP_TAG_OBJECT, .value={.selector=1, .value1={.tag=103, .f32=6.789}}}}})`)
    checkResult("size zero", () => peer.setSizeAttribute({ width: 0.0, height: 0.0 }),
        `setSize({.tag=INTEROP_TAG_OBJECT, .value={.width={.tag=INTEROP_TAG_OBJECT, .value={.selector=1, .value1={.tag=102, .i32=0}}}, .height={.tag=INTEROP_TAG_OBJECT, .value={.selector=1, .value1={.tag=102, .i32=0}}}}})`)
...
function checkOverloads() {
    ...
    //     `minSideBarWidth({.tag=102, .i32=42})`
    // )
...
function checkTabContent() {
    ...
    //     () => peer.tabBarAttribute(subTabBarStyle),
    ...
    //     () => peer.tabBarAttribute(bottomTabBarStyle),
...
function checkCanvasRenderingContext2D() {
    ...
    checkResult("new CanvasRenderingContext2D()",
        ...
        `new CanvasRenderingContext2D({.tag=INTEROP_TAG_UNDEFINED, .value={}}, {.tag=INTEROP_TAG_UNDEFINED, .value={}})[return reinterpret_cast<Ark_CanvasRenderingContext2D>(100)]getFinalizer()[return fnPtr<KNativePointer>(dummyClassFinalizer)]`)
    ...
    CanvasRenderingContext2D_serializer.write(serializer, unsafeCast<CanvasRenderingContext2D>(CanvasRenderingContext2DInternal.fromPtr(ctorPtr)))

    const deserializer = new DeserializerBase(serializer.toArray().buffer, serializer.length())
    const materializedBase = CanvasRenderingContext2D_serializer.read(deserializer) as unknown as MaterializedBase
