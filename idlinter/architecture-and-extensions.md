...
```
error[UnresolvedReference]: Unresolved reference
...
error[DuplicateIdentifier]: Duplicate identifier
...
Diagnostic-related types (with `DiagnosticMessage` being the main one) and formatting routines are moved to `idlize/core`. They are designed to allow further processing, and can be easily converted in LSP format if needed.

`idlize/core/src/messages.ts` contains templating (`DiagnosticMessageGroup`), and already declared templates like `ParsingFatal`. If you don't have matching instance of `DiagnosticMessageEntry` for your validation pass - create the new one there.

When all passes are finished - `outputReadableResult(idlManager.results)` is called and simple but powerfull formatting routines in `idlize/core/src/formatting.ts` are taking place. Messages are printed along with automatically counted `totals`.
